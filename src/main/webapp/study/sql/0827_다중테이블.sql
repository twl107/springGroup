...
-- 1번 이상 구매한 책들의 정보?(구매 날짜순 내림차순)
select * from orders;
select * from orders order by orderdate desc;

-- 1번 이상 구매한 책들의 정보?(구매 날짜순 내림차순) - 중복된 책 배제(1번 출력) - distinct, GROUP BY
...
-- 1번 이상 구매한 책들의 정보?(주문번호, 주문날짜, 책이름, 정가, 세일가) - 주문일자 내림차순(최근에 구매한 책 3권만 출력)
select o.orderid, o.orderdate, b.bookname, b.price, o.saleprice from orders o, books b where o.bookid = b.bookid limit 3;

-- 1번 이상 구매한 책들의 정보?(주문번호, 주문날짜, 책이름, 정가, 세일가) - 주문일자 내림차순(중복된 책은 1회만 출력)
...
select c.customer as 구매고객, format(sum(o.saleprice),0) as 총판매액
...
select b.bookname, b.publisher, count(o.bookid) as 총판매수 
	from orders o, books b
	where o.bookid = b.bookid and year(orderdate) = '2025'
	group by b.bookname;
	
/* ---------------------------------------- */
-- 서브쿼리(Sub Query) : 하나의 SQL문장안에서 또다른 SQL문을 포함시키는 것
...
*/
-- 도서를 한번이라도 구매한 적이 있는 고객명을 출력하시오.
...
select (select bookname from books where bookid=orders.bookid) as bookname from orders order by saleprice desc limit 1; 
